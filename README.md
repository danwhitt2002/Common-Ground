...
- **Landing page accent**: `assets/watercolor-flower.png` — a loose, hand-drawn loop-petal flower, its proportions measured directly off a reference photo (angle/length/width per petal), scaled up and thickened, run through an elastic warp for a hand-drawn wobble, then reduced to just its outer silhouette (no crossing lines through the middle) and filled flat solid orange (`#e06528`) — a block color, not a soft watercolour wash. Sits centered behind the hero content on `index.html` (`.landing-accent` in `styles.css`) at high opacity so the orange reads punchy against the navy — adjust `top`/`left`/`width`/`opacity` there to reposition, or swap the file for a different accent entirely. `assets/watercolor-cup.png` (an earlier soft-watercolour coffee cup treatment, in tan tones) is still in the repo, unused, if you'd rather switch back to it or use both.
