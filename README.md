...
- **Landing page accent**: `assets/watercolor-flower.png` — a loose, hand-drawn-style loop-petal flower (procedurally generated to match a reference image) rendered as a watercolour wash in orange tones against the navy, for contrast against the site's usual tan accent. Sits centered behind the hero content on `index.html` (`.landing-accent` in `styles.css`) at low opacity, just to fill the page's empty space — adjust `top`/`left`/`width`/`opacity` there to reposition, or swap the file for a different accent entirely. `assets/watercolor-cup.png` (the coffee cup sketch from the menu photo, same watercolour technique in tan tones) is still in the repo, unused, if you'd rather switch back to it or use both.
