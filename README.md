...
- **Landing page accent**: `assets/watercolor-cup.png` — the coffee cup sketch from the same menu photo, repainted as a watercolour wash (a soft painted corridor traced along the original ink line, organic bleeding edges, warm tan-to-terracotta tone variation, grain) rather than a flat line drawing. Sits behind the hero text on `index.html` (`.landing-accent` in `styles.css`) at low opacity, just to fill the page's empty space — adjust `top`/`right`/`width`/`opacity` there to reposition, or swap the file for a different accent entirely.
