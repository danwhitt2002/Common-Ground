...
- **Community graphic**: `assets/community-graphic.png` — a small huddle of people rendered in loose burnt-orange watercolor (real painted texture: pigment variation, soft edges, drips), symbolizing the club's community. Generated with an AI design tool rather than hand-coded shapes, background chroma-keyed out to transparent to match the site's exact navy. Used twice for continuity across every page: small, above the wordmark in the shared `<header class="topbar">` (`.topbar-graphic` in `styles.css`, appears on both `index.html` and `menu.html`), and larger as the landing page's hero accent (`.landing-accent`, `index.html` only) — adjust `top`/`left`/`width`/`opacity` on either rule to reposition, or swap the file for a different graphic (same filename in both places, or update the `<img src>` references). Three earlier accents (`assets/people-holding-hands.png`, flat pictogram figures clasping hands; `assets/watercolor-flower.png`, a loop-petal flower; `assets/watercolor-cup.png`, a soft-watercolour coffee cup) are still in the repo, unused, if you'd rather switch back to one of those instead.
